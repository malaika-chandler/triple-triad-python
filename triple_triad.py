...
class Rules:
    ...
    def handle_card_placement(self, challenger):
        ...
        for direction, neighbor in challenger.neighbors.items():
            if neighbor.has_card and neighbor.owner != challenger.owner:
                if self.should_card_be_flipped(challenger, neighbor):
                    neighbor.set_owner(challenger.owner)
                    count_cards_flipped += 1
    ...
    @staticmethod
    def should_card_be_flipped(challenger, neighbor):
        # TODO ensure neighbor plot has a card before comparison
        return challenger > neighbor
